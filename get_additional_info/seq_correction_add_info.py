...
def correction_and_add_cdrs(df, outfile_name):
    ...
    count how many sequences lost by failing anarci'''
    ...
    corrected_vh = corrected_vh[['VH', 'VL']]
    corrected_vl = pd.DataFrame(corrected_vl)
    corrected_vl = corrected_vl[['VH', 'VL']]
...
df = standardise_seqs(csv_file='patentoutput.csv', vh_col_name='HCVR', vl_col_name='LCVR')
final_df = correction_and_add_cdrs(df=df, outfile_name='test_generalised_code.csv')
