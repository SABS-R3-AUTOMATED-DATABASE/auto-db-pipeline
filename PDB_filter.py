...
# filter to check if (a) in PDB at all (verifies that really are PDB ID) and
# (b) in SAbDab (verifies that protein is an antibody)
...
for pdb_id in pdb_ids:
    ...
    # NOTE returning True even if PDB returns 'desired structure doesn't exist'
    if is_pdb_file_found is False:
        print(pdb_id, 'is not a PDB ID')
        pdb_ids.remove(pdb_id)
    # check against sabdab and remove if not an antibody
    # NOTE abdb not returning same results as sabdab web interface
    elif is_pdb_file_found is True:
