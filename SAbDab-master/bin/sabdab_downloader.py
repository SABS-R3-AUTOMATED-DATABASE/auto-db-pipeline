...
import argparse, sys, os, urllib.request, urllib.parse, urllib.error
...
def getchothpdb(pdb_entry, out_path):
    ...
    """
    out_file = os.path.join( out_path, "%s.pdb"%pdb_entry)
    urllib.request.urlretrieve("http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/structure/chothia/%s.pdb"%(pdb_entry,pdb_entry), out_file)
    if os.path.isfile(out_file):
        Retrieved = open(out_file).read()
        if not Retrieved.count("ATOM"):
            print("Failed to retrieve PDB file from SAbDab")
...
def getsequence(entry, fab_list, out_path):
    ...
    urllib.request.urlretrieve("http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/sequences/%s_raw.fa"%(entry,entry), out_file)
    if os.path.isfile(out_file):
        Retrieved = open(out_file).read()
    ...
    for fab in fab_list:
        ...
        if Hchain!="NA":
            ...
            urllib.request.urlretrieve("http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/sequences/%s_%s_VH.fa"%(entry,entry,Hchain), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
                if not Retrieved.count(">%s"%entry):
                    print("Failed to retrieve sequence file from SAbDab.")
        ...
        if Lchain!="NA":
            ...
            urllib.request.urlretrieve("http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/sequences/%s_%s_VL.fa"%(entry,entry,Lchain), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
                if not Retrieved.count(">%s"%entry):
                    print("Failed to retrieve sequence file from SAbDab.")
...
def getannotation(entry, fab_list, out_path):
    ...
    for fab in fab_list:
        ...
        if Hchain!="NA":
            ...
            urllib.request.urlretrieve("http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/annotation/%s_%s_VH.ann"%(entry,entry,Hchain), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
        ...
        if Lchain!="NA":
            ...
            urllib.request.urlretrieve("http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/annotation/%s_%s_VL.ann"%(entry,entry,Lchain), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
...
def getabangle(entry, fab_list, out_path):
    ...
    for fab in fab_list:
        if "NA" in fab:
            ...
            urllib.request.urlretrieve("http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/abangle/%s.abangle"%(entry,entry), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
...
def getimgt(entry, fab_list, out_path):
    ...
    for fab in fab_list:
        ...
        if Hchain!="NA":
            ...
            urllib.request.urlretrieve("http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/imgt/%s_%s_H.imgt"%(entry,entry,Hchain), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
        ...
        if Lchain!="NA":
            ...
            urllib.request.urlretrieve("http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/imgt/%s_%s_L.imgt"%(entry,entry,Lchain), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
                if not Retrieved.count("gene_type"):
                    print("Failed to retrieve imgt file from SAbDab.")
...
if __name__ == "__main__":
    ...
    if not (args.original_pdb or args.chothia_pdb or args.sequence or args.annotation or args.abangle or args.imgt):
        print("No requested data type given. Please choose at least one.", file=sys.stderr)
    ...
    try:
        ...
        print("A 'sabdab_dataset' already exists in the output directory. Please rename it or provide a new output directory.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print("Could not open summary file.", file=sys.stderr)
        sys.exit(1)
    except IndexError:
        print("Summary file in incorrect format.", file=sys.stderr)
        sys.exit(1)
