...
# Thomas 9/05/04
# Changed so that the chain identifiers can be upper or lower case - this is required for some structures. 
        ...
        if key=="TITLE":
            ...
            if len(tok)>=2:
                ...
                if ckey=='mol_id':
                    ...
                    dict['compound'][comp_molid][ckey]=re.sub("\A\s*","",re.sub("\;\s*\Z","",_chop_end_codes(tail)).split(":")[1])
                    last_comp_key=ckey
                else:
                    dict['compound'][comp_molid][ckey]=cval            
                    last_comp_key=ckey
            elif last_comp_key=="chain":
                dict['compound'][comp_molid][last_comp_key]+= re.sub("\;\s*\Z","",_chop_end_codes(tail)).split(":")[0]+" "
            else:
