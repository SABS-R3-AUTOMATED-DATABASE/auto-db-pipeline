...
class Interface:
    ...
    @property
    def output(self):
        """"Output of the PaperID for retrieval."""
        attrs = ('mention_ids', 'possible_ids')
        representation = {attr: getattr(self, attr) for attr in attrs}
        # representation.update(self._repr_ids)  # these need to be refactored anyways
    ...
    def get_possible_ids(self):
        """Function used to be called get_ids, before the simplication to possible.
        ...
        for id_name, regex in id_finding.items():
            ...
            self.possible_ids[id_name] = possible_id_values
